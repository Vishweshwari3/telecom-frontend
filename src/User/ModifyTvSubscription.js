...
const ModifyTvSubscription = () => {
  ...
  // Function to fetch filtered TV services from the backend
  const fetchFilteredServices = async () => {
    setLoading(true);
    setError(null);

    try {
      const response = await axios.get('http://localhost:8082/user/api/modify-tv-subscription', {
  ...
  const handleAction = async (service) => {
    ...
    try {
      ...
      alert('Error deactivating TV service.');
      console.error(err);
      return;
    }
